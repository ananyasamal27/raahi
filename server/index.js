...
// Help generate coordinates between points
...
app.post('/api/route', (req, res) => {
  ...
  const routes = ['safest', 'fastest', 'cheapest'].map(type => {
    ...
    // Map segments and generate waypoints
    let allWaypoints = [];
    const segments = routeData.modes.map((seg, idx) => {
      // Procedurally generate waypoints for this segment with slight variance per type
      const jitter = type === 'safest' ? 0.0002 : type === 'fastest' ? -0.0002 : 0.0004;
      const count = seg.mode === 'train' ? 12 : 5; // train journeys have more segments
...
// Map of socket.id -> { interval, currentLat, currentLng, elapsedSeconds }
...
io.on('connection', (socket) => {
  ...
  socket.on('start-guardian', (data) => {
    ...
    const interval = setInterval(() => {
      ...
      // Signal ranges from 1 to 5 bars
      const signal = Math.floor(Math.random() * 2) + 4; // 4-5 bars
...
// Wildcard fallback for React routing (ignore APIs)
